...
/**
 * Сколько ПРОФИЛЕЙ идёт в ведомость на метр линии прогона.
 *
 * Отношением массы сечения к массе профиля это не считается: у «[-]» оно
 * не целое (2,30…2,60), потому что сечение — два профиля ПЛЮС полоса
 * постоянного погонного веса (1,4211 или 2,0527 кг/м). Округление такого
 * отношения давало то 2, то 3, и на «[-]ПС145х45х1,2» завышало метраж в
 * полтора раза.
 *
 * Считает расчётчик так (ведомость 21604, где выбраны как раз «[-]»):
 *   ПС 145х45х1,5 = 8*2*12*2      — 8 рядов × 12 м зоны × 2 стены × 2 профиля
 *   ПС 145х45х1,2 = 7*2*6*2 + 7*2*10,7*2 + 5*2*37,3*2
 * то есть множитель ровно 2, а полоса в метраж не попадает вовсе — она
 * сидит только в массе (масса сечения × метры линии), и отдельной строкой
 * её не заказывают. Это воспроизводится как есть.
 */
const PROFILES_PER_SECTION: Record<string, number> = {
  "]": 1,
  "[]": 2,
  "][": 2,
  "[-]": 2,
...
export function profilesPerLine(profile: WallPurlinProfile): number {
  const known = PROFILES_PER_SECTION[profile.sectionType];
  if (known === undefined) {
    throw new Error(`Неизвестный тип сечения прогона: ${profile.sectionType}`);
  }
  return known;
}
