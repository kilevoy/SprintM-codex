...
export function buildBill(
  ...
  //
  // Стеновые прогоны выпадают только под сэндвич-панель. Под профнастил
  ...
  //
  // ВНИМАНИЕ: почему их снимают под сэндвичем — НЕ УСТАНОВЛЕНО. Раньше
  // здесь стояло «панель работает по стойкам сама и прогоны ей не нужны»;
  // инструкция по монтажу ТСП это опровергает: уплотнительная лента 3 мм
  // крепится «к наружным поверхностям колонн, балок, кровельных и
  // СТЕНОВЫХ ПРОГОНОВ», а панели ставятся вертикально на цоколь, то есть
  // горизонтальные опоры им нужны. Правило взято из постановки задачи и
  // держится на ней одной. Вопрос 8 раздела 12 аудита.
