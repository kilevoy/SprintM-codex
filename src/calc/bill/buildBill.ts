...
export function buildBill(
  ...
  //
  // ВНИМАНИЕ: фильтр снимает строки `ПС 145*`, но это НЕ стеновой прогон,
  // а ВОДОСТОК — см. комментарий в computeFrameExtras и вопрос 1в в
  // docs/ВОПРОСЫ_РАСЧЁТЧИКУ.md. Правило пришло из постановки задачи
  // («под сэндвичем прогоны исключаются») и было применено не к той
  // строке. Поведение оставлено как есть до ответа расчётчика: менять
  // состав поставки без подтверждения нельзя.
  //
  // Настоящие стеновые прогоны сэндвич-варианта (строка 34 раздела
  // «Стены», шаг 1200 мм) в SprintM пока не считаются вовсе, поэтому
  // снимать тут нечего. Под профнастил режим `frame-roof-profnastil`
  // секцию прогонов сохраняет — листу иначе не на что опираться.
